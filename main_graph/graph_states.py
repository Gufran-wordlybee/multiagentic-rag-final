...
class Router(TypedDict):
    ...
    logic: str
    type: Literal["more-info", "document_qa", "general"]
...
class AgentState(InputState):
    ...
    retry_decision: Optional[str] = field(default=None)
    """Set by human_approval_node from the resumed interrupt() value ('y'/'n').
    None means no interrupt has fired yet for the current turn. Read by the
    check_retry_decision conditional edge, which is a plain function with no
    side effects, as LangGraph requires for routing functions."""
